        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            if (Module::isInstalled('oyst') || Module::isEnabled('oyst')) {
                if ($payment_method != null && preg_match('/OneClick/i', $payment_method)) {
                    $cart_total_paid = (float)Tools::ps_round((float)$this->context->cart->getOrderTotalOyst(true, Cart::BOTH, null, null, true, $payment_method), 2);
                } else {
            ...
            foreach ($package_list as $id_address => $packageByAddress) {
                foreach ($packageByAddress as $id_package => $package) {
                    ...
                    if (Module::isInstalled('oyst') || Module::isEnabled('oyst')) {
                        if ($payment_method != null && preg_match('/OneClick/i', $payment_method)) {
                            $order->total_shipping_tax_excl = (float)$this->context->cart->getPackageShippingCostOyst((int)$id_carrier, false, null, $order->product_list, null, $payment_method);
                            $order->total_shipping_tax_incl = (float)$this->context->cart->getPackageShippingCostOyst((int)$id_carrier, true, null, $order->product_list, null, $payment_method);
                        } else {
